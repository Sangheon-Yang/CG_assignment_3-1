...
def display():
    global cameraIndex, cow2wld , cowCount, cow2wldList, cowLocationList, currentCowLocation ,cursorOnCowBoundingBox, C_ROM, animStartTime
    #print(cow2wld)
    ...
    #displayStartTime = glfw.get_time();
    ...
    if cowCount < 6:
        ...
        for cows in range(cowCount):
            drawCow(cow2wldList[cows], cursorOnCowBoundingBox)

    else:
        #print(cow2wldList)
        #print(cowLocationList)
        # for n in range(6):
        
        #print(animStartTime)
        #animTime=glfw.get_time()-animStartTime;
        #print(animTime)
        ti = glfw.get_time() - animStartTime
        if(ti>=18):
            cowCount=0
            cow2wld = cow2wldList[0]
            cowLocationList=[]
            cow2wldList=[]
            animStartTime = 0
            ...
            #startingPoint = cowLocationList[n]
            #while ti % 2 < 2.0:
            #t = ti/2.0
            
            L= np.array([t**3,t**2,t,1]) @ C_ROM
            
            #L= np.array([1,t,t*t,t*t*t]) @ C_Rom2
            P = np.array([cow2wldList[(n+5)%6].copy()[:3:,3],
                        cow2wldList[n%6].copy()[:3:,3],
                        cow2wldList[(n+1)%6].copy()[:3:,3],
                        cow2wldList[(n+2)%6].copy()[:3:,3]])
                # P = np.array([cow2wldList[(int(ti)+2)%6][:4:,3],
                #            cow2wldList[(int(ti)+1)%6][:4:,3],
                #            cow2wldList[int(ti)%6][:4:,3],
                #            cow2wldList[(int(ti)+5)%6][:4:,3]])
            #L= C_ROM@np.array([cowLocationList[(n-1+6)%6] , cowLocationList[(n+6)%6], cowLocationList[(n+1+6)%6], cowLocationList[(n+2+6)%6]])
                #3차원
                #print(Point)
                #setTranslation(cow2wld, startingPoint)
                #setTranslation(T, currentPos-pp.cowPickPosition)
                #cow2wld[0:3,3]=Point
            R = L@P
#tmpcow2wld = cow2wldList[n]
#       tmpcow2wld[:4:,3] = R

            cow = cow2wldList[(n)%6].copy()
            cow[:3:,3] = R
            
            
            
            
            newDV = np.array([3*t*t, 2*t, 1, 0]) @ C_ROM @ P
            
            #print("newDV")
            #print(newDV)
            x_axis = newDV
            #print("x")
            z_axis = np.cross(newDV, np.array([0,1,0]))
            #print("x")
            y_axis = np.cross(z_axis, x_axis)
                              
            nx_axis = (1.0/np.sqrt(np.dot(x_axis,x_axis))) * x_axis
            ny_axis = (1.0/np.sqrt(np.dot(y_axis,y_axis)))* y_axis
            nz_axis = (1.0/np.sqrt(np.dot(z_axis,z_axis)))* z_axis
            ...
            #Final = cow @ YR.T @ np.linalg.inv(nM).T
        ...
        #currentCowLocation = cowLocationList[0]
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG ,pickInfo ,cow2wld, currentCowLocation ,cowCount, cow2wldList, cowLocationList, cursorOnCowBoundingBox, animStartTime
    GLFW_DOWN=1;
    GLFW_UP=0;
    x, y=glfw.get_cursor_pos(window)
    if button == glfw.MOUSE_BUTTON_LEFT:
        ...
        if state == GLFW_DOWN:
            #print(2)
            if isDrag==0 and cursorOnCowBoundingBox:#initial state, first click
                #print(3)
                isDrag=0;
            elif isDrag==H_DRAG and cursorOnCowBoundingBox:
                #print(4)
                isDrag = V_DRAG;
            print( "Left mouse down-click at %d %d\n" % (x,y))
        # start vertical dragging
        elif state==GLFW_UP and isDrag ==0 and cursorOnCowBoundingBox:
            #print(5)
            isDrag = H_DRAG;
        elif state == GLFW_UP and isDrag!=0 and cursorOnCowBoundingBox:
            #print(6)
            if isDrag==V_DRAG and cursorOnCowBoundingBox :
                if cowCount<6:
                    cowCount +=1;
                    tmpcow2wld = cow2wld.copy()
                    cow2wldList.append(tmpcow2wld)
                    print("-----------------------------------------")
                    for i in cow2wldList:
                        print(i)
                        print("-----------------------------------------")
                    #cowLocationList.append(tmpcow2wld.T[3,0:3])
                    #print("-----------------------------------------")
                    #for j in cowLocationList:
                    #    print(j)
                    #    print("-----------------------------------------")
                    isDrag=H_DRAG;
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, currentCowLocation
    if isDrag: 
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                #c= pp.cowPickPosition;
                currentPos=ray.getPoint(c[1]);
                currentPos[0] = pp.cowPickPosition[0];
