...
def display():
    global cameraIndex, cow2wld , cowCount, cow2wldList, cowLocationList, currentCowLocation ,cursorOnCowBoundingBox, C_ROM
    #print(cow2wld)
    ...
    if cowCount < 7:
        ...
        for n in range(6):
            ...
            while ti < (3.0):
                ...
                P = np.array([cowLocationList[(n-1+6)%6],
                              cowLocationList[(n+6)%6],
                              cowLocationList[(n+1+6)%6],
                              cowLocationList[(n+2+6)%6]])
                ...
                #cow2wld[0:3,3]=Point
                tmpcow2wld = cow2wldList[n].copy()
                tmpcow2wld[0,3] = Point[0]
                tmpcow2wld[1,3] = Point[1]
                tmpcow2wld[2,3] = Point[2]
                
                drawCow(tmpcow2wld, cursorOnCowBoundingBox);
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG ,pickInfo ,cow2wld, currentCowLocation ,cowCount, cow2wldList, cowLocationList, cursorOnCowBoundingBox
    GLFW_DOWN=1;
    GLFW_UP=0;
    x, y=glfw.get_cursor_pos(window)
    if button == glfw.MOUSE_BUTTON_LEFT:
