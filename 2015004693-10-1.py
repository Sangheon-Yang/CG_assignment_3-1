...
def render():
    global gCamAng, gCamHeight, xang, zang1, zang2
    ...
    M = np.identity(4)
    Rz1 = np.array([[np.cos(np.radians( zang1)), -np.sin(np.radians(zang1)), 0],
                    [np.sin(np.radians(zang1)), np.cos(np.radians(zang1)), 0],
                    [0,0,1]])
    Rx = np.array([[1,0,0],
                   [0, np.cos(np.radians(xang)), -np.sin(np.radians(xang))],
                   [0, np.sin(np.radians(xang)), np.cos(np.radians(xang))]])
    Rz2 = np.array([[np.cos(np.radians(zang2)), -np.sin(np.radians(zang2)), 0],
                    [np.sin(np.radians(zang2)), np.cos(np.radians(zang2)), 0],
                    [0,0,1]])
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight, zang1, zang2, xang
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2015004693-10-1', None,None)
    if not window:
        glfw.terminate()
        return
