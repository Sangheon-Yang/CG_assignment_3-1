...
#    def get_pixel_color(self, surfacePoint , surfaceNormal , light)
...
def main():
    ...
    for c in root.findall('shader'):
        ...
        if(shader_type_c == 'Phong'):
            ...
            specularColor_c = np.array([-1.,-1.,-1.])
    ...
    for i in np.arange(imgSize[1]):
        for j in np.arange(imgSize[0]):
            ...
            if (randered_surface is not None and surfacePoint is not None):
                ...
                temp_pixel_color = pixel_shader.diffuseColor
            #print(temp_pixel_color)
            ...
            if(temp_T is not np.Infinity):
                for v in np.arange(number_of_light):
                    ...
                    if(not Blocked):
                        if(pixel_shader.is_phong_):
                            temp_pixel_color = max(0,np.dot(surfaceNormal,lightVector)) * light_arr[v].intensity *pixel_shader.diffuseColor + (max(0,np.dot(surfaceNormal,get_half_Vector(lightVector, -viewDir))) ** pixel_shader.exponent[0]) * light_arr[v].intensity*pixel_shader.specularColor
                                
                        else:
                            temp_pixel_color = max(0,np.dot(surfaceNormal,lightVector)) * light_arr[v].intensity *pixel_shader.diffuseColor
                                
                                
                        if(np.dot(np.array([1,1,1]), pixel_f_color) < np.dot(np.array([1,1,1]),temp_pixel_color)):
                            pixel_f_color = temp_pixel_color
            ...
            #print(surfaceNormal)
            #pixel_f_color = pixel_f_color + 0.1 * pixel_shader.diffuseColor
