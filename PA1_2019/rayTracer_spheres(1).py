...
class box:
    ...
    def get_T_in_box(self, udv , p):
        ...
        tmin_x = (self.minPt[0]-p[0])/udv[0]
        tmax_x = (self.maxPt[0]-p[0])/udv[0]
        if(tmin_x > tmax_x):
            swap(tmin_x , tmax_x)
        ...
        t_min = max(tmin_x , tmin_y)
        t_max = min(tmax_x , tmax_y)
        ...
        if(t_min > tmax_z or tmin_z > t_max):
            return 9999999.9

        t_min = max(t_min , tmin_z)
        t_max = min(t_max , tmax_z)
...
def main():
    ...
    for i in np.arange(imgSize[1]):
        for j in np.arange(imgSize[0]):
            ...
            pixelVector = get_pixelplane_vector(viewDir, viewWidth, viewHeight , imgSize[1], imgSize[0], projDistance, i, j, viewUp)
            temp_T = 9999999.9
