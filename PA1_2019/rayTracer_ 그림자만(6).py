    ...
    for i in np.arange(imgSize[1]):
        for j in np.arange(imgSize[0]):
            ...
            if (randered_surface is not None and surfacePoint is not None):
                ...
                #temp_diffuse_color = pixel_shader.diffuseColor
                #temp_specular_color = pixel_shader.specularColor
            #print(temp_pixel_color)
                ...
                pixel_shader = None
                    ...
                    #pixel_f_color = 0.1*temp_pixel_color
                ...
                for v in np.arange(number_of_light):
                    ...
                    if(not Blocked):
                        if(pixel_shader.type == "Phong"):
                            ...
                            print("LAMber" , temp_color)
                                
                    else: #block 됬을때, current 값을 가져온다?
                        temp_color = np.array([0,0,0])
